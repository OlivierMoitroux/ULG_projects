...
#                           DFS Search
#                           **********
# This pacman agent uses the depth-first-search algorithm
...
class PacmanAgent(Agent):
    ...
    def __init__(self, args):
        ...
        self.args = args
        # Stack items are paths. The last element of the path is the state of a node
        self._dfsStack = util.Stack()
    ...
    def _buildMoveSeq(self, state)->None:
        ...
        self._dfsStack.push(path)

        while not self._dfsStack.isEmpty():

            path = self._dfsStack.pop()

            # Retrieve the state (of the node != gameState) which is at the last position of the path
            currNodeState = path[len(path) - 1] # tuple <gameState, direction>
            currGameState = currNodeState[0]
            ...
            if currState not in self._visited:
                ...
                for successor in currGameState.generatePacmanSuccessors():
                    ...
                    if (successor[0].getPacmanPosition(), successor[0].getFood()) not in self._visited:
                        ...
                        self._dfsStack.push(successorPath)

        raise Exception("Stack empty: no solution found")
