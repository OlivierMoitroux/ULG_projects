...
#define FAT_IOCTL_LOCK_FS 1000
#define FAT_IOCTL_UNLOCK_FS 1001
#define FAT_IOCTL_CHPASSWD 1002
 ...
 * @return Fills the tokens array and returns 2 in case of background command
 * execution, 1 in case of variable definition (the first token is the name of
 * the variable, the second its value), 0 in case of standard command execution,
 * -1 if error
 */
 ...
 * in line
 * @param line : a ponter to the string to be filled with the result
 ...
 * @param retVal : address of variable to store the return value
 * @param retPid : address of variable to store the pid of the background
 * execution
 * @param backgrnd : background execution iff true
 * @return Sets ret to the return value and returns 0 in case of success, -2 if
 * exit cmd input or -1 otherwise
 */
static int exeLine(char *args[], int *retVal, pid_t *retPid, bool backgrnd);
 ...
 *
 */
static int bltIn_fat(char *args[]);

/**
 *
 */
static int bltIn_fatHide(char *args[]);

/**
 *
 */
static int bltIn_fatUnhide(char *args[]);

/**
 *
 */
static int bltIn_fatLock(char *args[]);

/**
 *
 */
static int bltIn_fatUnlock(char *args[]);

/**
 *
 */
static int bltIn_fatPassword(char *args[]);
...
/**
 * bltIn_sys_pfstat() : display statistics about a process id
 * @param args :  2d-array that stores the input cmds
 * @return 0 in case of success, -1 otherwise
 */
static int bltIn_sys_pfstat(char *args[]);
...
int main() {
    ...
    while (true) {
        ...
        // Execute cmd
            ...
            status = exeLine(args, &ret, &pid, backgrnd);
            if (status < 0) {
                free(line);
                // Error
                if (status == -1)
        ...
        //  Kill properly zombie processes if they finished
        for(int i = 0 ; i < nBackgrnd ; i++) {

            // non-blocking (WNOHANG) wait on children processes (-1)
...
int parseLine(char *tokens[], char *line) {
    ...
    if (curArg > 0 && strcmp(tokens[curArg - 1], "&") == 0) {
        tokens[curArg - 1] = (char*) NULL;
        return 2;
    }
...
int exeLine(char *args[], int *retVal, pid_t *retPid, bool backgrnd) {
...
#define NB_BLT_IN_CMD_FAT 5

static const fct bltInCmdFat[NB_BLT_IN_CMD_FAT] = {
    {bltIn_fatHide, "hide"},
    {bltIn_fatUnhide, "unhide"},
    {bltIn_fatLock, "lock"},
    {bltIn_fatUnlock, "unlock"},
    {bltIn_fatPassword, "password"}
};


int bltIn_fat(char *args[]) {
    // Arg check
    if (args[1] == NULL) {
        fprintf(stderr,"fat expects an argument\n\n");
        return -1;
    }
    // Arg 1 check
    for (int i = 0; i < NB_BLT_IN_CMD_FAT; i++) {
        if (strcmp(args[1], bltInCmdFat[i].name) == 0)
            return (*bltInCmdFat[i].fctPtr)(args);
    }
    // Unknowkn first argument
    fprintf(stderr, "fat: unknown argument %s", args[1]);
    return -1;
}


int bltIn_fatHide(char *args[]) {

    // Args check
    if (args[2] == NULL) {
        fprintf(stderr, "fat hide expects 1 argument\n");
        return -1;
    }
...
int bltIn_fatLock(char *args[]) {

    uint32_t id; // Unused parameter
    int fd;
    int ret;
    ...
    ret = ioctl(fd, FAT_IOCTL_LOCK_FS, &id);
    if (ret == -1) {
	perror("ioctl");
        close(fd);
	return -1;
...
int bltIn_fatUnlock(char *args[]) {
    ...
    ret = ioctl(fd, FAT_IOCTL_UNLOCK_FS, &passwd);
    if (ret == -1) {
	perror("ioctl");
        close(fd);
	return -1;
...
int bltIn_fatPassword(char *args[]) {

    // Args check
    if (args[2] == NULL) {
        fprintf(stderr, "fat password expects 1 argument\n");
        return -1;
    }
    ...
    // Get passwords (if one password given, the first is assumed to be none)
    if (args[3] == NULL) {
        passwds[0] = 0;
        passwds[1] = (uint32_t) atol(args[2]);
    } else {
        passwds[0] = (uint32_t) atol(args[2]);
        passwds[1] = (uint32_t) atol(args[3]);
    }
    ...
    ret = ioctl(fd, FAT_IOCTL_CHPASSWD, &passwds);
    if (ret == -1) {
	perror("ioctl");
        close(fd);
	return -1;
...
int bltIn_sys_pfstat(char *args[]) {

    // Args check
    if (args[2] == NULL) {
        fprintf(stderr, "sys pfstat expects 1 argument\n");
        printf("USAGE : sys ip addr DEV\n");
        return -1;
    }

    struct pfstat pfstatData;
    pid_t pid;
    if ((pid = atol(args[2])) == 0) {
        fprintf(stderr,"PID not a number\n");
        return -1;
    }

    // 377 is the system call number
    int ret = syscall(377, pid, &pfstatData);
    ...
    // Pid not valid
    if (ret == 1) {
        fprintf(stderr,"PID not valid\n");
        return -1;
    }

    // pfstatData not valid
    if (ret == 2) {
        fprintf(stderr,"Error: pfstat ptr not valid\n");
        return -1;
    }

    // Print data
    printf("stack_low %d\n", pfstatData.stack_low);
    printf("transparent_hugepage_fault %d\n", pfstatData.transparent_hugepage_fault);
    printf("anonymous_fault %d\n", pfstatData.anonymous_fault);
    printf("file_fault %d\n", pfstatData.file_fault);
    printf("swapped_back %d\n", pfstatData.swapped_back);
    printf("copy_on_write %d\n", pfstatData.copy_on_write);
    printf("fault_alloced_page %d\n", pfstatData.fault_alloced_page);
