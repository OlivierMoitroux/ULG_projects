...
class ContinuousCatcher():
	...
	def _drawImage(self, state):
		image = np.zeros((self.width, self.height))

		barWS = int(round(state[0] - self.bar_width / 2, 0))
		barWE = int(round(state[0] + self.bar_width / 2, 0))
		barHS = int(round(self.bar.center[1], 0))
		barHE = int(round(self.bar.center[1] + self.bar_height, 0))

		fruitWS = int(round(state[2] - self.fruit.size[0] / 2, 0))
		fruitWE = int(round(state[2] + self.fruit.size[0] / 2, 0))
		fruitHS = int(round(state[3] - self.fruit.size[1] / 2, 0))
		fruitHE = int(round(state[3] + self.fruit.size[1] / 2, 0))

		image[barHS:barHE, barWS:barWE] = 1
		image[fruitHS:fruitHE, fruitWS:fruitWE] = 0.5

		return image

	def generateAnimation(self, data):
		"""
		Generate a video of the catcher according to the given series
		of states and stat.
		"""
		...
		plt.axis('off')
		ims = []
		for idx in ProgressBar(range(len(data)), desc="Generating animation"):
			state = data[idx][0]
			lives = data[idx][1]

			image = self._drawImage(state)

			titleStr = "Lives = {}".format(lives)
			title = ax.text(0.5, 0.85, titleStr, ha="center",
							bbox={'facecolor':'w', 'alpha':0.7, 'pad':5},
							transform=ax.transAxes)
			im = plt.imshow(image, animated=True, cmap=cmap)
			ims.append([title, im])

		_ = animation.ArtistAnimation(fig, ims, interval=100, repeat_delay=1000)

		plt.show()
		
	def saveAnimation(self, data):
		"""
		Generate a video of the catcher according to the given series
		of states and stat.
		"""
		...
		_ = animation.ArtistAnimation(fig, ims, interval=100, repeat_delay=1000)
		_.save('animation.mp4', writer=writer)
