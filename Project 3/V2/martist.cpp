...
/*
* Very little changes have been made to parser.hpp. See @new.
* I only changed vector<string> to vector<Lexer::token> for efficiency reasons.
* The fact it is constructed via an istream in not a big deal and don't require much resources to "typecast" to.
*/
#include "parser.hpp"
#include "martist.hpp"
...
std::ostream &operator <<( std::ostream &output, Martist &m ) {
...
void Martist::paint(){
    try{
        ...
        */
        // _t is my convention to remind user that it's a custom type / custom structure / typedef
        exp_t redRPN, greenRPN, blueRPN; // See parser.hpp for more infos on how to use tools offered by this header
...
string Martist::randXpr(const int maxDepth){
    ...
    /*
    * Let's use a double linked list of token (so that ptr to last element is also stored)
    * It should be faster than vector to add at the front (no need to shift) in theory.
...
/**
* [Private] Evaluates a mathematical RPN expression of Lexer::token
...
*      - For efficiency reason (called width*height*3 times), I decided to use vector of token.
*        This allows 25~30% faster execution compared to vector<string> for 1000*1000 pixels.
*/
float Martist::computeXpr(vector<Lexer::token>& xpr,const float x, const float y){
    ...
    for(vector<Lexer::token>::size_type i = 0; i < xpr.size() ; ++i){

        if(xpr[i] == Lexer::X)
            oprdStack.push(x);

        else if(xpr[i] == Lexer::Y)
            oprdStack.push(y);

        else if (xpr[i] == Lexer::PI)
            oprdStack.push(M_PI);
            ...
            else if(xpr[i] == Lexer::COS){
                oprdStack.push(cos(b));
            }

            else if(xpr[i] == Lexer::TIMES){
                float a = oprdStack.top();
                oprdStack.pop();
                oprdStack.push(b * a);
            }

            else if(xpr[i] == Lexer::AVG){
                float a = oprdStack.top();
                oprdStack.pop();
                oprdStack.push((b + a)/2.0);
            }
        }
    }
