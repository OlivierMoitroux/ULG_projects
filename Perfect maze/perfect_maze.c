...
const int OPEN_H_0 = 0xFFFFFF00;
const int OPEN_H_1 = 0xFFFF00FF;
const int OPEN_H_2 = 0xFF00FFFF;
const int OPEN_H_3 = 0x00FFFFFF;
const int OPEN_V_0 = 0xFFFFFFE1;
const int OPEN_V_1 = 0xFFFFE1FF;
const int OPEN_V_2 = 0xFFE1FFFF;
const int OPEN_V_3 = 0xE1FFFFFF;
...
void connect(int* maze, int source, int dest, int nb_cols) {
	...
	if (dest - source > 1) { // open vertical connection (logique cfr indices)
		int mask; // Ce que l'on applique afin d'ouvrir une porte
		/*
		* Une ligne du laby = 8x8 mots de 32 bits. 1 cell= 1 byte.Word_offset représente donc 
		* le n° de la cell dans le mot actuel (entre 0 et 3)
		*/
		if (byte_offset == 0) { 
			mask = OPEN_V_0;
		...
		for (int i = 0; i < 2; ++i) { // four words to update
			maze[word_offset + i * WORDS_PER_MEM_LINE] &= mask; // AND bit par bit
		}
		...
		for (int i = 3; i < 7; ++i) { // two words to update
